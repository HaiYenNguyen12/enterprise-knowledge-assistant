...
class RagService:
  ...
  def answer_question(self, question: str, document_ids: list[str] | None = None):
    collection_info = self.qdrant_repository.get_collection()
    ...
    #   limit=3
    # )
    results = self.qdrant_repository.search_chunks(query_embedding.tolist(),document_ids = document_ids, limit=3)
